...
//    order by p.email, p.id
    ...
    public List<Solicitudacta> obtenerPorProyecto(int proyectoid){
        ...
        try{
            Query consulta = em.createQuery("SELECT sa FROM Solicitudacta sa WHERE sa.id IN (SELECT s FROM Solicitud s WHERE s.presupuestotarea.tarea.etapaid.proyectoid.id = :proyectoid)", Solicitudacta.class);
